...
FRONTEND_PROJECT_DIR="${FRONTEND_PROJECT_DIR:-${PROJECT_ROOT}}"
...
emit_report() {
  ...
  if [[ -x "${REPORT_SCRIPT}" ]]; then
    "${REPORT_SCRIPT}" --script-name "${script_name}" --stage "${stage_name}" --status "${status}" --version "${version}" --env "${target_env}" --message "${message}"
  fi
}
...
create_runtime_tmpdir() {
  ensure_dir "${TMP_ROOT}"
  RUNTIME_TMP_DIR="$(mktemp -d "${TMP_ROOT}/run.XXXXXX")"
}
...
validate_target_env() {
  case "$1" in
    dev|test|staging|prod) return 0 ;;
    *) return 1 ;;
  esac
}
...
contains_action() {
  local actions=",${RELEASE_ACTIONS:-publish},"
  [[ "${actions}" == *",$1,"* ]]
}

resolve_frontend_output_dir() { printf '%s' "${FRONTEND_PROJECT_DIR}/${FRONTEND_OUTPUT_DIR:-dist}"; }
resolve_bootstrap_command() { printf '%s' "${FRONTEND_BOOTSTRAP_CMD:-npm ci}"; }
resolve_validate_command() { printf '%s' "${FRONTEND_VALIDATE_CMD:-npm run type:check}"; }
resolve_build_command() { printf '%s' "${FRONTEND_BUILD_CMD:-npm run build}"; }
