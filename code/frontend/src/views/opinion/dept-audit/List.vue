...
import { listDeptAuditTasks } from '@/api/opinion/dept-audit'
import type { DeptAuditListItem } from '@/types/opinion/dept-audit'

defineOptions({ name: 'OpinionDeptAuditList' })
...
const total = ref(0)
const records = ref<DeptAuditListItem[]>([])

async function fetchData() {
  loading.value = true
  try {
    const res = await listDeptAuditTasks({
...
function goAudit(row: DeptAuditListItem) {
  router.push({ name: 'opinion-dept-audit-detail', params: { taskId: String(row.taskId) } })
}
...
const columns = computed(() => [
  ...
  { title: '征集模块', slotName: 'moduleName', width: 180 },
  { title: '提交状态', slotName: 'fillStatus', width: 110, align: 'center' as const },
...
<template>
  <section class="opinion-dept-audit-list">
    ...
    <div class="action-bar">
      <h3 class="page-title">专业部门反馈审核</h3>
    ...
    >
      ...
      <template #fillStatus="{ record }">
        <a-tag :color="fillStatusColor(record.submitStatus)" size="small">{{ record.submitStatusText }}</a-tag>
      </template>
